         ...
         <table className="w-full " style={{borderSpacing:"20px"}}>
           <thead>
             <tr className="text-left ">
                    ...
                    <th ><div onClick={()=>sortByName()} className='cursor-pointer p-2'>Username</div></th>
                    <th ><div onClick={()=>sortByEmail()} className='cursor-pointer p-2'>Email</div></th>
           ...
           <tbody>
            {sortedUsers?sortedUsers.map(user=>(
              <tr key={user._id} className="text-left text-base font-normal">
               <th className='font-normal'> 
                <div className='flex items-center'>
                    <input type="checkbox" className='cursor-pointer' checked={handleChecked(user._id)} onChange={((e)=>handleSellect(user._id,e.target.checked))} />
               ...
               <th className="font-normal p-2">
                  <div className="flex items-center">
                    ...
                    <img className="w-full h-full object-cover" src={user.profilePic} alt="na" />
                  </div>
                    <div className="">
               ...
               <th  className="font-normal p-2 flex items-center py-4">
                 <Link to={{pathname:`/dashboard/user/${user._id}`,user:user}} className="bg-green-500 cursor-pointer w-fit px-2 rounded-lg text-white">
                   Edit
                   </Link>
                   <div onClick={()=>DeleteUser(user._id)} className="text-2xl cursor-pointer w-fit px-2 rounded-lg text-red-500">
                     <MdDeleteOutline/>
                   </div>
               </th>
             </tr>
